...
bool SVDBiasPredictor::init_user_item_line() {
    ...
    for (int i=0; i<num_items; ++i)
        item_popular[i] = pow(item_popular[i] + 1.0, 0.5);
    ...
    for (int i=0; i<num_users; ++i)
        user_activity[i] = pow(user_activity[i] + 1.0, 0.5);
    ...
    for (int i=0; i<num_items; ++i)
        item_popular_sns[i] = pow(item_popular_sns[i] + 1.0, 0.5);
    ...
    for (int i=0; i<num_users; ++i)
        user_activity_sns[i] = pow(user_activity_sns[i] + 1.0, 0.5);
