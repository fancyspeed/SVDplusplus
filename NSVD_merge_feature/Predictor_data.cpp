...
void Predictor::parse_user_line(string& str) {
    ...
    int tab = str.find("\t", 0);
    int tab1 = str.find("\t", tab + 1);
    int tab2 = (int) str.length();
    int cur_user;

    string userid_s = str.substr(0, tab);
    string age_s = str.substr(tab+1, tab1-tab-1);
    string gender_s = str.substr(tab1+1, tab2-tab1-1);

    if (userid_2_idx.end() == userid_2_idx.find(userid_s)) {
        cur_user = userid_idx ++;

        userid_2_idx[userid_s] = cur_user;
        user_age[cur_user] = atoi(age_s.c_str());
        user_gender[cur_user] = atoi(gender_s.c_str());
    }
}
